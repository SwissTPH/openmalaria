...
public:
	...
	// Infection parameters shouldn't be used; check by setting to NaNs.
    ...
    void testNewInf () {
	TS_ASSERT_EQUALS (infection->getDensity(), 0.00000000000000000);
    }
	...
	TS_ASSERT_DELTA (infection->getDensity(), 15.36758760023472284, 1e-8);
    }
    void testUpdated2Inf () {
	infection->updateDensity (Global::simulationTime + 1, 1.0);
	infection->updateDensity (Global::simulationTime + 2, 1.0);
	TS_ASSERT_DELTA (infection->getDensity(), 4.94261787639103382, 1e-8);
    }
    void testUpdated3Inf () {
	infection->updateDensity (Global::simulationTime + 1, 1.0);
	infection->updateDensity (Global::simulationTime + 2, 1.0);
	infection->updateDensity (Global::simulationTime + 3, 1.0);
	TS_ASSERT_DELTA (infection->getDensity(), 162.62062791268144860, 1e-7);
    }
	...
	TS_ASSERT_DELTA (infection->getDensity(), 6.10393200785528424, 1e-8);
    }
	...
	TS_ASSERT_DELTA (infection->getDensity(), 0.49426178763910338, 1e-8);
    }
	...
	TS_ASSERT_DELTA (infection->getDensity(), 1.97582432565095644, 1e-8);
    }
