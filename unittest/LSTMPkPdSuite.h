...
public:
    ...
    void testOral () {
	proxy->medicate (MF_index, 3000, 0, NaN, massAt21);
	...
	proxy->medicate (MF_index, 1500, 0, 0, massAt21);
	proxy->medicate (MF_index, 1500, 0, 0, massAt21);
	TS_ASSERT_APPROX (proxy->getDrugFactor (proteome_ID), 0.03564073617400945);
    }
    ...
    void testOralSplit () {
	proxy->medicate (MF_index, 3000, 0, NaN, massAt21);
	proxy->medicate (MF_index, 0, 0.5, NaN, massAt21);	// insert a second dose half way through the day: forces drug calculation to be split into half-days but shouldn't affect result
	TS_ASSERT_APPROX (proxy->getDrugFactor (proteome_ID), 0.03564073617400945);
    }
    ...
    void testIVEquiv () {
        ...
        proxy->medicate (MF_index, 3000/massAt21, 0, 1e-6, massAt21);
        TS_ASSERT_APPROX (proxy->getDrugFactor (proteome_ID), 0.03564073617400945);
    }
    
    void testIV () {
        // IV over whole day
        proxy->medicate (MF_index, 50, 0, 1, massAt21);
        TS_ASSERT_APPROX (proxy->getDrugFactor(proteome_ID), 0.10315895127530212);
    }
    
    void testIVSplit (){
        // As above, but split into two doses
        proxy->medicate (MF_index, 25, 0, 0.5, massAt21);
        proxy->medicate (MF_index, 25, 0.5, 0.5, massAt21);
        TS_ASSERT_APPROX (proxy->getDrugFactor(proteome_ID), 0.10315895127530212);
    }
    
    void testCombined (){
        proxy->medicate (MF_index, 50, 0, 0.5, massAt21);
        proxy->medicate (MF_index, 1500, 0.5, NaN, massAt21);
        TS_ASSERT_APPROX (proxy->getDrugFactor(proteome_ID), 0.05850739976088096);
    }
    
    void testSimultaneous (){
        proxy->medicate (MF_index, 1500, 0, NaN, massAt21);
        proxy->medicate (MF_index, 50, 0, 0.5, massAt21);
