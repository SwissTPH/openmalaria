...
public:
    ...
    void testConstant () {
        ...
        DecayFuncHet dHet;
        TS_ASSERT_EQUALS( df->evalAge( sim::fromDays(5), dHet ), 0.0 );
        // Second test: with an appropriately sampled helper value, we should get the results we want
        dHet = df->hetSample();
        TS_ASSERT_APPROX( df->evalAge( sim::zero(), dHet ), 1.0 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(10), dHet ), 1.0 );
        // Third test: time of decay (age plus now) should always be in the future
        TS_ASSERT_EQUALS( df->sampleAgeOfDecay(), sim::future() );
    }
    ...
    void testStep () {
        ...
        dHet = df->hetSample();
        TS_ASSERT_APPROX( df->evalAge( sim::zero(), dHet ), 1.0 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(6), dHet ), 1.0 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(20), dHet ), 0.0 );
        TS_ASSERT_EQUALS( df->sampleAgeOfDecay(), sim::fromYearsI(10) );
    }
    ...
    void testLinear () {
        ...
        dHet = df->hetSample();
        TS_ASSERT_APPROX( df->evalAge( sim::zero(), dHet ), 1.0 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(6), dHet ), 0.4 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(20), dHet ), 0.0 );
    }
    ...
    void testExponential () {
        ...
        dHet = df->hetSample();
        TS_ASSERT_APPROX( df->evalAge( sim::zero(), dHet ), 1.0 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(6), dHet ), 0.65975394736842108 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(20), dHet ), 0.25 );
    }
    ...
    void testWeibull () {
        ...
        TS_ASSERT_APPROX( df->evalAge( sim::zero(), dHet ), 1.0 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(6), dHet ), 0.73631084210526321 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(20), dHet ), 0.122306 );
    }
    ...
    void testHill () {
        ...
        dHet = df->hetSample();
        TS_ASSERT_APPROX( df->evalAge( sim::zero(), dHet ), 1.0 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(6), dHet ), 0.6936673684210527 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(20), dHet ), 0.24805074736842106 );
    }
    ...
    void testSmoothCompact () {
        ...
        dHet = df->hetSample();
        TS_ASSERT_APPROX( df->evalAge( sim::zero(), dHet ), 1.0 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(6), dHet ), 0.40656965789473687 );
        TS_ASSERT_APPROX( df->evalAge( sim::fromYearsI(20), dHet ), 0.0 );
    }
