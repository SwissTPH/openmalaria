 ...
 *
 * Each human has an instance for each type of drug present in their blood. */		// KW - Is the code in this class used for each drug in a human. i.e. if there are two drugs present is this section of code implemented twice?				
...
public:
  ...
  double calculateDrugFactor(const ProteomeInstance* infProteome, double ageYears, double weight_kg); /* KW- returns the total drug factor for one drug over one day
																											 I assume this is called multiple times (once for each drug present?)
																											 Where are the return values for this function stored? 
																											 The drug factor values generated by this function must be multiplied to reflect the drug action of all drugs in one day? */
  /// Checkpointing
  template<class S>
