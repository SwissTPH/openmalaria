...
Model* create(const scnXml::Scenario &scenario)
{
    ...
    model->population = std::unique_ptr<Population>(population);
    model->transmission = std::unique_ptr<Transmission::TransmissionModel>(transmission);
    model->surveyOnlyNewEp = surveyOnlyNewEp;
    model->humanWarmupLength = humanWarmupLength;
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        std::unique_ptr<Model> model = unique_ptr<Model>(create(*scenario));
        ...
        else
        {
            ...
            run(*model, endTime, estEndTime, "Warmup");
            ...
            while(iterate > sim::zero())
            {
                ...
                run(*model, endTime, estEndTime, "EIR Calibration");
        ...
        // Main phase loop
        run(*model, endTime, estEndTime, "Intervention period");
