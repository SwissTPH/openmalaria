...
// 
// Lambda: GSL vector of GSL vectors
// Lambda[t][0] = Nv0[t], Lambda[t][i] = 0 for i > 0
// 
...
// emergence rate (FMosqEmergeRateVector is OUT)
...
// calc Upsilon, assuming one human host
// (Upsilon, PAPtr, and PAiPtr are OUT)
...
// Calc diff of sim Sv to output, using root finding (f is OUT)
int CalcSvDiff_rf(const gsl_vector* x, void* p, gsl_vector* f);

// Calc siff of sim Sv to output (SvDiff is OUT)
